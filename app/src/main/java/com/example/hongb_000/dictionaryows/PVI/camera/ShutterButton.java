...
package com.example.hongb_000.dictionaryows.PVI.camera;
 ...
 * It's currently an {@code ImageView} that can call HistoryActivity delegate when the
...
public class ShutterButton extends ImageView {
	/**
	 * A callback to be invoked when HistoryActivity ShutterButton's pressed state changes.
	 */
	public interface OnShutterButtonListener {
		/**
		 * Called when HistoryActivity ShutterButton has been pressed.
		 *
	 ...
	 protected void drawableStateChanged() {
		 ...
		 if (pressed != mOldPressed) {
			 if (!pressed) {
				 ...
				 // button. When clicking using HistoryActivity trackball button, the view
