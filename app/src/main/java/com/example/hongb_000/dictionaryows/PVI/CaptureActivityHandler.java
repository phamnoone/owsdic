...
final class CaptureActivityHandler extends Handler {
  ...
  CaptureActivityHandler(CaptureActivity activity, CameraManager cameraManager, boolean isContinuousModeActive) {
    ...
    if (isContinuousModeActive) {
      ...
      // Display HistoryActivity "be patient" message while first recognition request is running
  ...
  public void handleMessage(Message message) {
    ...
    switch (message.what) {
      ...
      case R.id.ocr_decode_failed:
        state = State.PREVIEW;
        activity.setShutterButtonClickable(true);
        Toast toast = Toast.makeText(activity.getBaseContext(), "Nhận diện lỗi, Xin vui lòng thử lại.", Toast.LENGTH_SHORT);
  ...
  void stop() {
    // TODO See if this should be done by sending HistoryActivity quit message to decodeHandler as is done
    // below in quitSynchronously().
    Log.d(TAG, "SettingActivity state to CONTINUOUS_PAUSED.");
    ...
    // TODO are these removeMessages() calls doing anything?
...
//    CameraManager.get().stopPreview();
  ...
  void resetState() {
    ...
    if (state == State.CONTINUOUS_PAUSED) {
      Log.d(TAG, "SettingActivity state to CONTINUOUS");
      state = State.CONTINUOUS;
      restartOcrPreviewAndDecode();
    }
  }
  ...
  void quitSynchronously() {    
    ...
    try {
      //quit.sendToTarget(); // This always gives "sending message to HistoryActivity Handler on HistoryActivity dead thread"
      
      // Wait at most half HistoryActivity second; should be enough time, and onPause() will timeout quickly
  ...
  /**
   *  Send HistoryActivity decode request for realtime OCR mode
   */
